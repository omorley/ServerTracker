...
// Bonus round
		System.out.println("Missing character level is: " + getLevelOfMissingCharacter());
	...
	private static int getRandomLevel() {
		return rand.nextInt(100) + 1; // TODO is this right?
	}
	...
	public static ArrayList<Character> rollCharacters() {
...
//		}
	...
	private static void checkForPandaren(ArrayList<Character> characters) throws DontHaveTheExpansionException {
		...
		while (iterator.hasNext()) {
			...
			if (character.getRace() == Race.PANDAREN) {
				throw new DontHaveTheExpansionException("Error, we don't have the expansion!");
			}
		}
	...
	public static int getLevelOfMissingCharacter() {
		...
		int xorvalue = 0;
		for (Character character: characters) {
			xorvalue = xorvalue ^ character.getLevel();
		}
		int expectedXor = 0;
		for (int i = 1; i <= 85; i++) {
			expectedXor = expectedXor ^ i; 
		}
		int missinglevel = xorvalue ^ expectedXor;
		return missinglevel;
	}
